...
void TemplateManagerDialog::setupConnections() {
  connect(addButton, &QPushButton::clicked, this,
          &TemplateManagerDialog::addTemplate);
  connect(saveButton, &QPushButton::clicked, this,
          &TemplateManagerDialog::saveTemplate);
  connect(deleteButton, &QPushButton::clicked, this,
          &TemplateManagerDialog::deleteTemplate);
  connect(addConditionButton, &QPushButton::clicked, this,
          &TemplateManagerDialog::addConditionalArgument);
  connect(manageVariantsButton, &QPushButton::clicked, this,
          &TemplateManagerDialog::openVariantManager);
  connect(templateList, &QListWidget::itemClicked, this,
          &TemplateManagerDialog::onTemplateSelected);
}

// Set templates and update the template list
void TemplateManagerDialog::setTemplates(
    const QList<CommandTemplate> &templates) {
...
void TemplateManagerDialog::addTemplate() {
  QString newTemplateName =
      QInputDialog::getText(this, "New Template", "Template Name:");
...
void TemplateManagerDialog::saveTemplate() {
  if (selectedTemplateIndex >= 0 &&
      selectedTemplateIndex < commandTemplates.size()) {
    commandTemplates[selectedTemplateIndex].templateName = templateName->text();
    commandTemplates[selectedTemplateIndex].commandStructure =
        commandStructure->toPlainText();
    commandTemplates[selectedTemplateIndex].category = templateCategory->text();

    // Save placeholders and conditional arguments
    commandTemplates[selectedTemplateIndex].placeholders =
        extractPlaceholdersFromTree();
    updateTemplateList();
  }
}
...
void TemplateManagerDialog::addConditionalArgument() {
  if (selectedTemplateIndex < 0 ||
      selectedTemplateIndex >= commandTemplates.size()) {
    QMessageBox::warning(this, "No Template Selected",
  ...
  if (dialog.exec() == QDialog::Accepted) {
    ...
    if (!condition.isEmpty()) {
      commandTemplates[selectedTemplateIndex].conditionalArguments.append(
          condition);
      QMessageBox::information(this, "Condition Added",
                               "Conditional argument added successfully.");
    }
  }
}
...
void TemplateManagerDialog::openVariantManager() {
  ...
  if (variantDialog.exec() == QDialog::Accepted) {
    commandTemplates[selectedTemplateIndex].variants =
        variantDialog.getVariants();
  }
}

// Extract placeholders from a command structure
QStringList TemplateManagerDialog::extractPlaceholdersFromStructure(
...
void TemplateManagerDialog::populatePlaceholderTree(
...
void TemplateManagerDialog::updateTemplateList() {
  ...
  for (int i = 0; i < commandTemplates.size(); ++i) {
    QListWidgetItem *item =
        new QListWidgetItem(commandTemplates[i].templateName, templateList);
